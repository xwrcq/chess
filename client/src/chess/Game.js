...
function minimaxW(game, depth, isMaximizingPlayer, sum, color) {
  positionCount++;
  let children = game.moves({ verbose: true });
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    let currPrettyMove = game.move(currMove);
    let newSum = evaluateBoard(game, currPrettyMove, sum, color);
    let [childBestMove, childValue] = minimaxW(
      game,
      depth - 1,
      !isMaximizingPlayer,
      newSum,
...
function getBestMove(game, color, currSum) {
  ...
  let alg = localStorage.getItem("alg");
  let [bestMove, bestMoveValue] =
    alg === "a"
      ? minimax(
          game,
          depth,
      ...
      : minimaxW(game, depth, true, currSum, color);

  let d2 = new Date().getTime();
  let moveTime = d2 - d;
  let positionsPerS = (positionCount * 1000) / moveTime;

  console.log("moveTime: ", moveTime, "ms");
  localStorage.setItem("time", moveTime);
  console.log("positionCount: ", positionCount);
  localStorage.setItem("pos", positionCount);
...
export function move(from, to, promotion) {
  ...
  try {
    ...
    if (legalMove) {
      ...
      let historyMove = history.length ? `,${legalMove.from}-${legalMove.to}` : `${legalMove.from}-${legalMove.to}`;
      history.push(historyMove);
      localStorage.setItem("hist", JSON.stringify(history));

      window.setTimeout(function () {
        let botMove = getBestMove(chess, "b", globalSum)[0];
        chess.move(botMove);
        updateGame();
        history.push(`,${botMove.from}-${botMove.to}`);
        localStorage.setItem("hist", JSON.stringify(history));
      }, 250);
    }
